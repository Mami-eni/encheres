...
<body>
	...
	<nav class="navbar navbar-light bg-light">
		<div class="container-fluid justify-content-between">
			<a href="./encheres"> <img alt="logo" src="img/logo.png">
			</a>
			<c:choose>
				<c:when test="${empty sessionScope.user }">
					<div>
						...
						<a class="btn btn-outline-warning me-2"
							href="${pageContext.request.contextPath}/connection">Se
							connecter</a>
					</div>
				</c:when>
				...
				<c:otherwise>
					<div>
						<a class="btn btn-outline-success me-2"
							...
							href="${pageContext.request.contextPath}/encheres">Vendre un
							article</a> <a class="btn btn-outline-warning me-2"
							href="${pageContext.request.contextPath}/monprofil">Mon
							profil</a> <a class="btn btn-outline-warning me-2"
					...
					<form method="post"
						...
						<c:if test="${!empty sessionScope.user }">
							...
							<fieldset>
								...
								<input type="checkbox" name="flitreCheckboxAchat" value="1"
									${disabledAchat}>1<br> <input type="checkbox"
									name="flitreCheckboxAchat" value="2" ${disabledAchat}>2<br>
								<input type="checkbox" name="flitreCheckboxAchat" value="3"
									${disabledAchat}>3<br> <br>
							</fieldset>
							<fieldset>
								<legend>ventes</legend>
								<input type="checkbox" name="flitreCheckboxVente" value="1"
									${disabledVentes}>1<br> <input type="checkbox"
									name="flitreCheckboxVente" value="2" ${disabledVentes}>2<br>
								<input type="checkbox" name="flitreCheckboxVente" value="3"
									${disabledVentes}>3<br> <br>
													...
													<div class="card-body">
														...
														<c:choose>
															<c:when test="${empty sessionScope.user}">
																...
																<p class="card-text">${enchere.getArticle().nom}
																	${enchere.getArticle().description}</p>
															...
															<c:otherwise>
																...
																<a class="card-text"
																	href="${pageContext.request.contextPath}/encheres?article=${enchere.getArticle().numero}">${enchere.getArticle().nom}
																	${enchere.getArticle().description}</a>
																<p class="card-text">${enchere.montant}</p>
																<p class="card-text">${enchere.getArticle().dateFinEncheres}</p>
																<a class="card-text"
																	href="${pageContext.request.contextPath}/afficherProfil?vendeur=${enchere.getArticle().getUtilisateur().numero}">${enchere.getArticle().getUtilisateur().pseudo}</a>
